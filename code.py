...
class unit:
    def __init__(self,root):
        ...
        self.root.geometry('465x500+300+50')
        ...
        #==========Entry============
        textUnitID = Entry(innerframe, font=fonts, textvariable="", width=30)
        textUnitID.grid(row=0, column=1)


        lblUnitName = Label(innerframe, font=fonts, text='Unit Name', width=10, justify=RIGHT)
        lblUnitName.grid(row=1, column=0)
        textUnitID = Entry(innerframe, font=fonts, textvariable="", width=30)
        textUnitID.grid(row=1, column=1)
        ...
        self.member_record = ttk.Treeview(detailframe, height=12, columns=("UnitID", "UnitName"), yscrollcommand=scrollY.set)
